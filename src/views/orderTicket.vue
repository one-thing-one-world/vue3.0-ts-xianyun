  ...
  <div class="flex jc" style="margin-top:5px">
    <div class="orderBox flex jsb">
      <div class="BoxOne">
        ...
        <div v-if="errss===true" class="err">证件号码格式错误</div>
        <div class="btn">
...
export default defineComponent({
  ...
  setup(props, ctx: SetupContext) {
    ...
    const kong = (): void => {
      ...
      const flegsss = data.arr.every((item: any) => {
        const reg = /^[1-9]\d{5}(?:18|19|20)\d{2}(?:0[1-9]|10|11|12)(?:0[1-9]|[1-2]\d|30|31)\d{3}[\dXx]$/;
        // console.log(reg.test(item.value3));
        return reg.test(item.value3) === true;
      });
      // console.log(flegsss);
      ...
      if (flegs === false && flegss === false && flegsss === false) {
        data.err = false;
        data.errs = false;
        data.errss = true;
      }
      if (flegs === false && flegss === false && flegsss === true) {
        data.err = false;
        data.errs = false;
        data.errss = false;
      }
    ...
    onMounted(() => {
      ...
      data.setId = route.query.setId as string;
